...
export async function SiteHeader() {
  ...
  const desktopMenuTextClassName = session
    ? "text-xs uppercase tracking-[0.18em] text-stone-700 transition hover:text-stone-950"
    : "text-sm uppercase tracking-[0.25em] text-stone-700 transition hover:text-stone-950";
  const desktopUserTextClassName = session
    ? "hidden xl:inline text-[13px]"
    : "hidden xl:inline";
  ...
  return (
    <SiteHeaderShell>
      <div className="lg:hidden">
        ...
        <div className="header-surface bg-[#f8f4ed] px-4 py-5 transition-colors duration-300">
      ...
      <div className="header-surface hidden bg-[#f8f4ed] transition-colors duration-300 lg:block">
          ...
          <nav className="hidden items-center gap-6 lg:flex">
            ...
            {visibleMenus.map((menu) => (
              <div key={menu.id} className="group relative">
                <Link
                  href={menu.href}
                  className={desktopMenuTextClassName}
                >
                  {menu.name}
                </Link>
            ...
            <div className="hidden items-center gap-2 text-stone-700 md:flex">
              {session ? (
                <Link href="/profile" className="inline-flex items-center gap-2 text-sm text-stone-700">
                  ...
                  <span className={desktopUserTextClassName}>Bienvenido: {session.name}</span>
                </Link>
              ) : (
                <div className="inline-flex items-center gap-2 text-sm text-stone-700">
                  <UserRound className="h-5 w-5" />
                  <span className={desktopUserTextClassName}>Bienvenido: Invitado</span>
